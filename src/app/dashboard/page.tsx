...
function fmtEur(val: any, decimals = 0): string {
  const n = parseFloat(val);
  if (isNaN(n)) return 'â';
  return n.toLocaleString('de-DE', { minimumFractionDigits: decimals, maximumFractionDigits: decimals }) + ' â¬';
}
...
// âââ Mini Sparkline Chart (SVG) ââââââââââââââââââââââââââââââââââââââââââââââ
...
// âââ Bar Chart (horizontal) ââââââââââââââââââââââââââââââââââââââââââââââââââ
...
// âââ Page 1: Gesamtlage ââââââââââââââââââââââââââââââââââââââââââââââââââââââ
    ...
    <div>
      ...
      <div style={{
        ...
        <span style={{ fontSize: '1.25rem' }}>
          {status === 'GREEN' ? 'ð¢' : status === 'YELLOW' ? 'ð¡' : 'ð´'}
        </span>
        <div>
      ...
      <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
        <KpiCard title="Umsatz" value={fmtEur(revenue)} sub={`${trendIcon(revMom)} ${fmtPct(revMom)} ggÃ¼. Vormonat`} icon="ð" />
        <KpiCard title="EBIT" value={fmtEur(profit)} sub={`${trendIcon(profitMom)} ${fmtPct(profitMom)} ggÃ¼. Vormonat`}
          color={profit < 0 ? '#ef4444' : profit === 0 ? 'var(--text-primary)' : '#22c55e'} icon="ð°" />
        <KpiCard title="Marge" value={fmtPct(marginPct)} sub={marginPct < 0.05 ? 'Unter Ziel' : marginPct < 0.1 ? 'Im Bereich' : 'Gut'}
          color={marginPct < 0.05 ? '#ef4444' : marginPct < 0.1 ? '#eab308' : '#22c55e'} icon="ð" />
        {productivity > 0 && <KpiCard title="ProduktivitÃ¤t" value={fmtPct(productivity)} sub="Auslastung" icon="âï¸" />}
      </div>
...
// âââ Page 2: Vertragsanalyse ââââââââââââââââââââââââââââââââââââââââââââââââââ
...
function Page2({ data: rawData }: { data: any }) {
  ...
  return (
    <div>
      {/* Summary KPIs */}
      <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
        <KpiCard title="VertrÃ¤ge gesamt" value={String(totalContracts || 'â')} icon="ð" />
        <KpiCard title="Kritische VertrÃ¤ge" value={String(criticalContracts || 'â')}
          color={criticalContracts > 0 ? '#ef4444' : '#22c55e'} icon="â ï¸" />
        <KpiCard title="Portfolio-EBIT" value={fmtEur(portfolioEbit)}
          color={portfolioEbit < 0 ? '#ef4444' : '#22c55e'} icon="ð¼" />
        {avgMargin !== 0 && <KpiCard title="Ã Marge" value={fmtPct(avgMargin)} icon="ð" />}
      </div>

      {/* Contract List */}
      {contracts.length > 0 && (
        <>
          <SectionHeader title={`VertragsÃ¼bersicht (${contracts.length} VertrÃ¤ge)`} />
...
// âââ Page 3: LiquiditÃ¤tsstabilitÃ¤t ââââââââââââââââââââââââââââââââââââââââââ
...
function Page3({ data: rawData }: { data: any }) {
  ...
  return (
    <div>
      {/* KPI Cards */}
      <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
        <KpiCard title="Bankbestand" value={fmtEur(bankBalance)} icon="ð¦"
          color={bankBalance < 0 ? '#ef4444' : undefined} />
        <KpiCard title="LiquiditÃ¤tsreichweite" value={liquidityMonths > 0 ? `${fmt(liquidityMonths, 1)} Monate` : 'â'}
          color={liquidityColor} icon="ð"
          sub={liquidityMonths < 1 ? 'Kritisch' : liquidityMonths < 3 ? 'Warnung' : 'Stabil'} />
        {avgCost > 0 && <KpiCard title="Ã Monatliche Kosten" value={fmtEur(avgCost)} icon="ð¸" />}
        {status && <KpiCard title="Status" value={statusLabel(status)} color={statusColor(status)} icon={status === 'GREEN' ? 'ð¢' : status === 'YELLOW' ? 'ð¡' : 'ð´'} />}
      </div>

      {/* LiquiditÃ¤ts-Gauge */}
      {liquidityMonths > 0 && (
        <>
          <SectionHeader title="LiquiditÃ¤tsreichweite" />
          <div className="card">
            ...
            <div style={{ marginTop: '0.75rem', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
              {liquidityMonths < 1
                ? 'â ï¸ Kritisch: LiquiditÃ¤t reicht weniger als 1 Monat'
                : liquidityMonths < 3
                  ? 'â¡ Warnung: LiquiditÃ¤tsreserve unter 3 Monaten'
                  : 'â Stabil: Ausreichende LiquiditÃ¤tsreserve'}
            </div>
          </div>
        </>
      )}
      ...
      {scoreTotal > 0 && (
        <>
          <SectionHeader title={`StabilitÃ¤ts-Score: ${fmt(scoreTotal)}/100`} />
          <div className="card">
            {[
              { label: 'Leistung', value: scorePerf },
              { label: 'Struktur', value: scoreStruct },
              { label: 'Trend', value: scoreTrend },
              { label: 'StabilitÃ¤t', value: scoreStab },
...
// âââ Page 4: MaÃnahmen & Benchmarks ââââââââââââââââââââââââââââââââââââââââ
...
function Page4({ data: rawData }: { data: any }) {
  ...
  return (
    <div>
      {/* Summary KPIs */}
      <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
        <KpiCard title="MaÃnahmen gesamt" value={String(actions.length || 'â')} icon="ð" />
        <KpiCard title="Gesamtpotenzial" value={totalPotential > 0 ? fmtEur(totalPotential) : 'â'}
          color="#3b82f6" icon="ð¡" />
        {focusImpact > 0 && <KpiCard title="Monatsfokus" value={fmtEur(focusImpact)} color="#22c55e" icon="ð¯" />}
        {highRiskCount > 0 && <KpiCard title="Risiko-VertrÃ¤ge" value={String(highRiskCount)} color="#ef4444" icon="â ï¸" />}
      </div>
        ...
        <>
          ...
          <div className="card" style={{ background: 'rgb(239,246,255)', borderColor: 'rgb(191,219,254)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <span style={{ fontSize: '2rem' }}>ð¯</span>
              <div>
                <div style={{ fontWeight: 700, fontSize: '1.1rem', color: 'rgb(30,58,138)' }}>
                  {focus.action_label || focus.contract_name || focus.action_key || 'Aktuelle MaÃnahme'}
                </div>
                {focusImpact > 0 && (
                ...
                {focus.prioritaet && (
                  <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: 2 }}>
                    PrioritÃ¤t: {focus.prioritaet} | {focus.kategorie || ''}
                  </div>
                )}
              </div>
            </div>
      ...
      {actions.length > 0 && (
        <>
          <SectionHeader title={`MaÃnahmenpool (${actions.length} MaÃnahmen)`} />
          <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
              <thead>
                <tr style={{ background: 'var(--background)', borderBottom: '1px solid var(--border-color)' }}>
                  <th style={{ padding: '0.75rem 1rem', textAlign: 'left', fontWeight: 600, color: 'var(--text-secondary)' }}>MaÃnahme</th>
                    ...
                    <tr key={i} style={{ borderBottom: '1px solid var(--border-color)', background: isFocus ? 'rgb(239,246,255)' : undefined }}>
                      <td style={{ padding: '0.75rem 1rem' }}>
                        <div style={{ fontWeight: isFocus ? 700 : 500 }}>
                          {isFocus && <span style={{ marginRight: 6 }}>ð¯</span>}
                          {a.action_label || a.contract_name || a.action_key || a.kategorie || `MaÃnahme ${i + 1}`}
                        </div>
                      ...
                      <td style={{ padding: '0.75rem 1rem', textAlign: 'right', fontWeight: 600, color: potential > 0 ? '#22c55e' : 'var(--text-primary)' }}>
                        {potential > 0 ? `+${fmtEur(potential)}` : 'â'}
                      </td>
                      <td style={{ padding: '0.75rem 1rem', textAlign: 'center' }}>
                        {score > 0 && (
                          <span style={{
      ...
      {actions.length === 0 && benchmarks.length === 0 && Object.keys(focus).length === 0 && (
        <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
          ...
          <p style={{ color: 'var(--text-secondary)' }}>MaÃnahmen werden geladen...</p>
        </div>
      )}
    </div>
  );
...
// âââ Main Dashboard Page âââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const initCustomers = async () => {
      ...
      // Admin with empty customers list â try API, fallback to known customers
      if (!customers) {
        try {
      ...
      <div className="card">
        {loading ? (
          ...
          <div className="text-center py-12">
            <p style={{ color: 'var(--text-secondary)' }}>
              {selectedCustomer && selectedPeriod ? 'Keine Daten verfÃ¼gbar' : 'Bitte Kunde und Periode auswÃ¤hlen'}
            </p>
          </div>
        )}
      </div>
