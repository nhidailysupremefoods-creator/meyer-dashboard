...
const WORKFLOW_STEPS: { type: DocumentType; label: string; icon: string; sentKey: keyof OperationsCustomer; driveUrl: string; driveLabel: string }[] = [
  { type: 'angebot',    label: 'Angebot',      icon: '📄', sentKey: 'angebot_sent',    driveUrl: 'https://drive.google.com/file/d/1rYW2jPtpjt935kq2KxLKvsEc5W204CNJ/view', driveLabel: 'Begleitemail_Angebot.docx' },
  { type: 'vertrag',    label: 'DL-Vertrag',   icon: '📝', sentKey: 'vertrag_sent',    driveUrl: 'https://drive.google.com/file/d/1ut8qAndtgrsDtGLOxgidA2gR1C6dkHw-/view',  driveLabel: 'Begleitemail_Vertraege.docx' },
  { type: 'unterlagen', label: 'Unterlagen',    icon: '📎', sentKey: 'unterlagen_sent', driveUrl: 'https://drive.google.com/file/d/1AtzEIdGCEpWL55Gemg9MLI2StgCh21tt/view',  driveLabel: 'All-In-One_Email_nach_Vertragsunterzeichnung.docx' },
  { type: 'reminder',   label: 'Reminder',      icon: '🔔', sentKey: 'reminder_sent',   driveUrl: 'https://drive.google.com/file/d/14NXQq-oUzwJJqBTM8nkEiz6cYXfHJ-bM/view',  driveLabel: 'Email_Reminder_Datenupload.docx' },
  { type: 'rechnung',   label: 'Rechnung',      icon: '💶', sentKey: 'rechnung_sent',   driveUrl: 'https://drive.google.com/file/d/1jh8SHjKm7jcKvUrPhdmHmlzK0o7BJZ3q/view',  driveLabel: 'Email_Rechnung_Begleitmail.docx' },
];
...
// Gmail-Verifikation: Mapping von DocumentType → API-Schlüssel
const GMAIL_KEY_MAP: Record<DocumentType, string> = {
  angebot:    'angebot',
  vertrag:    'vertrag',
...
export default function OperationsPage() {
  ...
  // gmailStatus: Map von customer_id → { angebot: bool, vertrag: bool, ... }
  ...
  async function verifyGmailStatus(customer: OperationsCustomer) {
    ...
    try {
      ...
      // Speichere Gmail-Status für diesen Kunden
      setGmailStatus(prev => ({ ...prev, [customer.customer_id]: data }));

      // Aktualisiere customer-Flags falls Gmail-Status abweicht
      ...
      // Stille Fehler – zeige lokalen Status wenn API nicht erreichbar
    } finally {
      setVerifyingId(null);
    }
  }

  // ── Toggle row expand + Gmail-Verifikation ──────────────
  function handleRowClick(customer: OperationsCustomer, expanded: boolean) {
    if (expanded) {
      // Zuklappen
      setExpandedId(null);
    } else {
      // Aufklappen + Gmail prüfen
      setExpandedId(customer.customer_id);
      verifyGmailStatus(customer);
    }
  }
    ...
    switch (type) {
      ...
      case 'unterlagen':
        return `<div style="font-family:Arial,sans-serif;color:#192231;line-height:1.6;">
          ...
          <p><a href="https://meyer-dashboard-v2.vercel.app" style="color:#B08A6A;">https://meyer-dashboard-v2.vercel.app</a></p>
      ...
      case 'rechnung':
        return `<div style="font-family:Arial,sans-serif;color:#192231;line-height:1.6;">
          ...
          <p>Die Rechnung bezieht sich auf die vereinbarte Advisory-Leistung inklusive Zugang zum webbasierten Dashboard sowie die laufende wirtschaftliche Analyse und Einordnung der Kennzahlen.</p>
    ...
    <div>
      ...
      <div className="space-y-3">
        {customers.map(customer => {
          ...
          const isVerifying = verifyingId === customer.customer_id;
          const customerGmail = gmailStatus[customer.customer_id];
          return (
            <div key={customer.customer_id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div
                ...
                onClick={() => handleRowClick(customer, expanded)}
              >
              ...
              {expanded && (
                <div className="px-6 pb-6 pt-2 border-t border-gray-100">
                  {/* Gmail-Verifikation Ladeindikator */}
                  {isVerifying && (
                  ...
                  {/* Gmail-Verifikation Hinweis wenn abweichend */}
                  {customerGmail && !isVerifying && (
                  ...
                  <div className="grid grid-cols-5 gap-4">
                    {WORKFLOW_STEPS.map(step => {
                      ...
                      // Gmail-Status für diesen Schritt (falls verfügbar)
                      const gmailKey = GMAIL_KEY_MAP[step.type];
                      const gmailSent = customerGmail ? customerGmail[gmailKey] : undefined;
                      // Zeige "Entwurf" wenn lokal als gesendet markiert, aber Gmail keinen Versand bestätigt
                      const isDraftOnly = isSent && gmailSent === false;

                      return (
                        <div key={step.type} className="text-center">
                          <div className="text-2xl mb-1">{step.icon}</div>
                          <div className="text-[11px] font-semibold text-gray-500 mb-1">{step.label}</div>
                          <a
                            href={step.driveUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            title={step.driveLabel}
                            className="inline-block text-[10px] text-copper hover:underline mb-2"
                          ...
                          {isDraftOnly && (
                            <div className="text-[10px] text-amber-600 font-medium mb-2" title="Entwurf erstellt, aber noch nicht abgesendet">
                              ⚠ Entwurf – nicht gesendet
                            </div>
                          )}
                          {isSent && gmailSent === undefined && (
                            <div className="text-[10px] text-green-600 font-medium mb-2">✓ Gesendet</div>
                          )}
