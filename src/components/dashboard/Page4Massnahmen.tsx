...
function BenchmarkGauge({ label, current, targetMin, targetMid, targetMax, isProxy, monthlyRevenue }: {
  ...
  let barColor = 'var(--border-color)', statusText = 'Keine Daten', statusBg = 'rgba(255,255,255,0.06)', statusColor = '#999';
  if (hasValue) {
    if (lowerIsBetter) {
  ...
  return (
    <div>
      <div style={{ padding: '1.75rem 2rem 1.25rem', maxWidth: 1400, margin: 0 }}>
        ...
        <p style={{ fontSize: '0.95rem', color: 'var(--text-secondary)', margin: 0, fontWeight: 400 }}>Steuerungs-Cockpit — Optimierungspotenziale erkennen, priorisieren und umsetzen</p>
      </div>
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {liqLevers.map((lever, i) => {
            ...
            return (
              <div key={i} className="p-4 rounded-xl relative" style={{ border: lever.biggest ? '2px solid #C8A96E' : '1px solid var(--border-color)', background: 'var(--navy, #192231)', paddingTop: lever.biggest ? '1.5rem' : '1rem' }}>
        ...
        <div className="card">
          ...
          <div className="space-y-2">
            {ebitActions.map((action: any, idx: number) => {
              ...
              return (
                <div key={action.action_key || idx} className="p-3.5 rounded-xl" style={{ border: '1px solid var(--border-color)', background: 'var(--navy, #192231)' }}>
                    ...
                    <tr key={item.action_key} style={{ borderTop: '1px solid var(--border-color)', background: isAutoCompleteCandidate ? 'rgba(16,185,129,0.04)' : undefined }}>
                      ...
                      <td className="py-3 px-3">
                        {trackerTab === 'aktiv' ? (
                          <select
                            ...
                            style={{ background: 'var(--navy, #192231)', color: 'var(--text-primary)', borderColor: 'var(--border-color)', cursor: 'pointer' }}
                          >
                      ...
                      <td className="py-3 px-3">
                        <input type="text" value={item.note} onChange={e => handleUpdateStatus(item.action_key, { note: e.target.value })} placeholder="–"
                          className="text-xs border rounded px-2 py-1" style={{ width: 80, borderColor: 'var(--border-color)', color: 'var(--text-primary)', background: 'var(--navy, #192231)' }} />
                      </td>
