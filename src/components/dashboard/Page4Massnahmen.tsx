...
function BenchmarkGauge({ label, current, targetMin, targetMid, targetMax, isProxy, monthlyRevenue }: {
  ...
  return (
    <div className="card">
      ...
      <div className="relative h-4 rounded-full" style={{ backgroundColor: 'var(--background-card, #162236)' }}>
        ...
        <div className="absolute top-0 h-4" style={{ left: `${pctMid}%`, width: 2, backgroundColor: 'var(--text-primary, #E8ECF1)', borderRadius: 1 }} />
      </div>
  ...
  return (
    <div>
      <div style={{ padding: '1.75rem 2rem 1.25rem', maxWidth: 1400, margin: 0 }}>
        ...
        <p style={{ fontSize: '0.95rem', color: 'var(--text-secondary, #7B8FA8)', margin: 0, fontWeight: 400 }}>Steuerungs-Cockpit — Optimierungspotenziale erkennen, priorisieren und umsetzen</p>
      </div>
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {liqLevers.map((lever, i) => {
            ...
            return (
              <div key={i} className="p-4 rounded-xl relative" style={{ border: lever.biggest ? '2px solid #C8A96E' : '1px solid var(--border-color)', background: 'var(--background-card, #162236)', paddingTop: lever.biggest ? '1.5rem' : '1rem' }}>
                {lever.biggest && <span className="absolute text-xs font-bold rounded px-2 py-0.5" style={{ top: -11, left: 16, background: '#C8A96E', color: 'var(--background-card, #162236)' }}>GRÖSSTER HEBEL</span>}
                <div className="flex justify-between items-start mb-2">
        ...
        <div className="card">
          ...
          <div className="space-y-2">
            {ebitActions.map((action: any, idx: number) => {
              ...
              return (
                <div key={action.action_key || idx} className="p-3.5 rounded-xl" style={{ border: '1px solid var(--border-color)', background: 'var(--background-card, #162236)' }}>
                  <div className="flex items-center gap-3">
                    <div className="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold" style={{ background: '#C8A96E', color: 'var(--background-card, #162236)' }}>{rank}</div>
      ...
      <div className="card">
        ...
        <div className="space-y-2">
          {poolActions.map((action: any, idx: number) => {
            ...
            return (
              <div key={key} onClick={() => handleTogglePool(action)} className="flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all" style={{ border: `1px solid ${selected ? '#C8A96E' : action.isBiggest ? '#C8A96E' : 'var(--border-color)'}`, background: selected ? 'rgba(200,169,110,0.05)' : 'var(--background-card, #162236)' }}>
                <div className="flex-shrink-0 w-5 h-5 rounded border-2 flex items-center justify-center transition-all" style={{ borderColor: selected ? '#C8A96E' : 'var(--border-color, #1E3350)', background: selected ? '#C8A96E' : 'var(--background-card, #162236)' }}>
                ...
                {liqLevers.map((lever, i) => {
                  ...
                  return (
                    <div key={leverKey} className="flex items-center gap-3 p-3 rounded-xl" style={{ border: '1px solid var(--border-color)', background: 'rgba(0,0,0,0.02)', opacity: 0.7 }}>
                      <span className="flex-shrink-0 px-1.5 py-0.5 rounded text-xs font-bold" style={{ background: 'rgba(33,150,243,0.12)', color: '#1565C0' }}>LIQ</span>
                      <div className="flex-1 min-w-0">
      ...
      <div className="card">
        <div className="flex items-center gap-2 mb-4">
          ...
          {kpis.carry_over_count > 0 && (
            <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(33,150,243,0.12)', color: '#1565C0' }}>
        ...
        {displayItems.length > 0 ? (
                    ...
                    <tr key={item.action_key} style={{ borderTop: '1px solid var(--border-color)', background: isAutoCompleteCandidate ? 'rgba(16,185,129,0.04)' : undefined }}>
                      ...
                      <td className="py-3 px-3">
                        {trackerTab === 'aktiv' ? (
                          <select
                            ...
                            style={{ background: 'var(--background-card, #162236)', color: 'var(--text-primary)', borderColor: 'var(--border-color)', cursor: 'pointer' }}
                          >
                        ...
                        <div className="flex items-center gap-2">
                          {trackerTab === 'aktiv' ? (
                            <input type="range" min={0} max={100} step={5} value={item.realization}
                              ...
                              className="flex-1" style={{ accentColor: item.realization >= 80 ? '#2E8B57' : item.realization >= 40 ? '#C8A96E' : 'var(--text-secondary, #7B8FA8)', height: 6 }} />
                          ) : (
                            <div className="flex-1 relative h-2.5 rounded-full" style={{ backgroundColor: 'var(--background-card, #162236)' }}>
                      ...
                      <td className="py-3 px-3">
                        <input type="text" value={item.note} onChange={e => handleUpdateStatus(item.action_key, { note: e.target.value })} placeholder="–"
                          className="text-xs border rounded px-2 py-1" style={{ width: 80, borderColor: 'var(--border-color)', color: 'var(--text-primary)', background: 'var(--background-card, #162236)' }} />
                      </td>
          ...
          <div className="text-center py-8 rounded-xl" style={{ background: 'var(--background, #0B1526)', border: '1px dashed var(--border-color)' }}>
